 ...
 *	- Drift correction is now automatically performed on the intensity data
 *	
 *	Version 4.5:
...
maxNrHits = 500;		//Maximum number of hits in a single .rgn file
...
//			if (microscope == "Confocal TCSPC / TauSeparation") run("Bio-Formats Importer", "open=["+file_list[f]+"] color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT series_"+s+1);
		...
		tileSizeX = (width * pixelWidth)/1E6;
		tileSizeY = (height * pixelHeight)/1E6;
...
function segment_cells_no_nuclei(intensity_stack) {
	//Segment cells using Cellpose and nuclei using Stardist
	selectWindow(intensity_stack);
	getDimensions(width, height, channels, slices, frames);
